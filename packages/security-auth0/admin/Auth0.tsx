...
import { LoginScreenRenderer, useTenancy } from "@webiny/app-serverless-cms";
...
const AppClientIdLoader: FC<AppClientIdLoaderProps> = ({ auth0, rootAppClientId, children }) => {
    ...
    const authRef = useRef<FC | null>(null);
    ...
    return loaded ? React.createElement(authRef.current as FC, {}, children) : null;
};
...
export const Auth0: FC<Auth0Props> = React.memo(props => {
    const LoginScreenPlugin = createComponentPlugin(LoginScreenRenderer, () => {
        return function Auth0LoginScreen({ children }) {
            return (
                <AppClientIdLoader auth0={props.auth0} rootAppClientId={props.rootAppClientId}>
                    {children}
                </AppClientIdLoader>
            );
        };
